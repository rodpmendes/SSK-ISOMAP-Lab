"""
High-resolution visualization for experimental results.

Generates one figure per metric (ri, ch, fm, v, dbs, ss), with 3 columns per page layout.
Colors:
...
def extract_metric(results: Dict, metric_index: int, normalized: bool) -> Dict[str, Dict[str, float]]:
    """Extract metric values for each method and dataset.

    Args:
        results (Dict): Experimental results dictionary.
        metric_index (int): Index of the metric (0=ri, 1=ch, etc.).
        normalized (bool): Whether to use normalized datasets.
    ...
    for dataset_key in results:
        ...
        for method, metrics_list in results[dataset_key].items():
            try:
                extracted[dataset_name][method] = metrics_list[metric_index][0]
            except (IndexError, TypeError):
                extracted[dataset_name][method] = None  # Handle missing metric
    return extracted


def plot_single_metric(metric_name: str, metric_index: int, results: Dict, normalized: bool, output_dir: str, base_filename: str) -> None:
    """Plot a single metric across datasets and methods.

    Args:
        metric_name (str): Name of the metric (e.g., 'ri').
        metric_index (int): Index of the metric in result lists.
    ...
    """
    data = extract_metric(results, metric_index, normalized)
    ...
    # Custom legend (only showing KISOMAP and SSKISOMAP)
...
def main() -> None:
    ...
    results_file = os.path.join('old', 'dataset_results_v1.json')
    output_dir = os.path.join('old', 'img')
    ...
    for metric_idx, metric_name in enumerate(metrics):
        plot_single_metric(metric_name, metric_idx, results, normalized=False, output_dir=output_dir, base_filename=base_filename)
        plot_single_metric(metric_name, metric_idx, results, normalized=True, output_dir=output_dir, base_filename=base_filename)
